...
threading.current_thread().name = 'Main_L'
mask_path = ['/mnt/data1/mvi2/pred_train2','/mnt/data1/mvi2/pred_test2']
OTT=['m_results_train2.csv','m_results_test2.csv']
data_path = '/mnt/data1/mvi2/nrrd_set_new3'
PARAMS = 'RadiomicsParams.yaml'# Parameter file
    ...
    for outfile,inpath in zip(OTT,mask_path):
        ...
        try:
            for i, name in enumerate(patients):
                ...
                seg = glob.glob(os.path.join(data_path,  name + '_segs_*.nrrd'))

                ismvi = datas[0].split('/')[-1].split('_')[-1].split('.')[0]
                #f = h5py.File(data[0], 'r')
               # datas = f['raw']
                datas.remove(seg[0])
                #seg = glob.glob(os.path.join('/mnt/data1/mvi2/liverseg', name + '.nrrd'))
                row['Image']=datas
