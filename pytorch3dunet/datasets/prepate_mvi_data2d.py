...
output_path='/mnt/data1/mvi2/c2d-gbz'
...
for p in patients:
    ...
    for i in range(6):
        if i==0:
            ...
            bg=(center-np.max([width,np.array([0,112,112])],0)).astype(np.int)
            ed=(center+np.max([width,np.array([0,112,112])],0)).astype(np.int)

        temp = sitk.GetArrayFromImage(C[i])
        ...
        ww=np.zeros((max(zz.max()-1,zz.min()+2)-zz.min()-1,512,512))
        ww[:,:temp.shape[1],:temp.shape[2]]=temp[zz.min()+1:max(zz.max()-1,zz.min()+2),:temp.shape[1],:temp.shape[2]]
        R.append(ww)
        padding=[[0,0],
    ...
    C = np.array(C)
    R = np.array(R)
    padding_c=np.zeros((6,1,C.shape[1],224,224))
    ...
    f.create_dataset(name='cropped',data=np.array(padding_c, dtype='float32'))
    f.create_dataset(name='raw',data=np.array(R, dtype='float32'))
    #f.create_dataset(name='len',data=np.array(R, dtype='uint8'))
