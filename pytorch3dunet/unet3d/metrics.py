    ...
    def __call__(self, input, target):
        if isinstance(input,list):
            ...
            for i,aseg in enumerate(input):
               aseg = (aseg > 0).long()
               alldice.append(compute_per_channel_dice(aseg, target[:,i:i+1,:,:,:], epsilon=self.epsilon))
...
class BlobsAveragePrecision(GenericAveragePrecision):
    ...
    def __init__(self, thresholds=None, metric='ap', min_instance_size=None, input_channel=-1, **kwargs):
