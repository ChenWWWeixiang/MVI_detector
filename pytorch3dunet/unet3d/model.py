...
class Abstract3DUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
                 ...
                 conv_kernel_size=3, pool_kernel_size=2, conv_padding=1,multi_head=False, **kwargs):
        ...
        if self.multi_head:
            self.head_numbers=10
            self.mu=torch.ones((f_maps[0]))*torch.tensor(np.random.randn(f_maps[0])*0.1+1)
            self.mu=self.mu.cuda()
            self.bias = torch.ones((f_maps[0]))*torch.tensor(np.random.rand(f_maps[0]))
            self.bias = self.bias.cuda()
            self.final_conv = nn.ModuleList([nn.Conv3d(f_maps[0], out_channels+1, 1)]*self.head_numbers)
            #self.pcler=nn.Conv3d(1,1,1)
            #self.p_fc=nn.Linear(self.head_numbers,self.head_numbers)
        else:
    ...
    def forward(self, x):
        ...
        if self.multi_head:
            X=[]
            P=[]
            for n_head in range(self.head_numbers):
                t=x*self.mu.reshape((1,-1,1,1,1))+self.bias.reshape((1,-1,1,1,1))
                pm=self.final_conv[n_head](t.float())
                p=torch.nn.functional.adaptive_avg_pool3d(pm[:,-1,:,:,:],(1,1,1)).view(pm.shape[0],-1)
                pm = pm[:, :-1, :, :, :]
                X.append(pm)
                P.append(p)
            P=torch.cat(P,1).softmax(-1)
            #P=self.p_fc(P).sigmoid()
            x=torch.cat(X,1)
        else:
            x = self.final_conv(x)
...
class CLSNet3D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        self.encoders = nn.ModuleList(encoders)
        self.final_conv = nn.Linear(f_maps[-1], out_channels, 1)
    ...
    def forward(self, x):
        ...
        x=self.final_conv(torch.nn.functional.adaptive_avg_pool3d(encoders_features).flatten().relu())
        return x
...
def get_model(config):
    ...
    return model_class(**model_config,multi_head=config['multi_head'])
...
class ResNetV2(nn.Module):
    ...
    def __init__(self, block_units, width_factor):
        ...
        self.root = nn.Sequential(OrderedDict([
            ('conv', StdConv2d(6, width, kernel_size=7, stride=2, bias=False, padding=3)),
