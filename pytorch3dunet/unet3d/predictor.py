...
class StandardPredictor(_AbstractPredictor):
    ...
    def post_process(self,mask_prediction):
        labels = measure.label(mask_prediction, connectivity=2)
        max_num=0
        max_pixel=0
        for j in range(1, np.max(labels) + 1):
            ...
            if np.sum(labels == j) > 0.3 * np.sum(labels != 0):
    ...
    def _save_results(self, prediction_maps, normalization_masks, output_heads, output_file, dataset):
        ...
        for prediction_map, normalization_mask, prediction_dataset in zip(prediction_maps, normalization_masks,
            ...
            prediction_map=(prediction_map[1,:,:,:]>0.5).astype(np.uint8)
            #prediction_map = self.post_process(prediction_map)
            prediction_map=sitk.GetImageFromArray(prediction_map)
            sitk.WriteImage(prediction_map,output_file.filename.replace('h5','nrrd'))
