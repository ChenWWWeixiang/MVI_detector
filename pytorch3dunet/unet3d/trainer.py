...
class MyThread(threading.Thread):
    def __init__(self, a, b, ):
        ...
        #self.name = name
        self.extractor = RadiomicsFeatureExtractor('../radiomics_work/RadiomicsParams.yaml')
        self.img = a
        self.mask = b
    def run(self):
        try:
...
class UNet3DTrainer:
                ...
                if not self.skip_train_validation:
                    if self.multi_head:
                        ...
                        for j in range(self.head_num):
                            eval_score += self.eval_criterion(output[:,j*2:j*2+2,:,:,:], target)
        ...
        with torch.no_grad():
            for i, t in enumerate(val_loader):
                ...
                if self.multi_head:
                    eval_score = torch.zeros(1).cuda()
                    for j in range(self.head_num):
                        eval_score += p[:,j]*self.eval_criterion(output[:, j * 2:j * 2 + 2, :, :, :], target)
                    eval_score /= p.sum()
                else:
                    eval_score = self.eval_criterion(output, target)
    ...
    def clf_loss(self,r_features,cls_gt,p):
        ...
        for b in range(r_features.shape[0]):
            for pre_mask in range(r_features.shape[1]):
                ...
                pred=torch.tensor([1-pred,pred]).cuda().reshape((cls_gt.shape[0],-1)).float()
                loss+=torch.nn.functional.cross_entropy(pred,cls_gt)*p[b,pre_mask]
    ...
    def _forward_pass(self, input, target, cls=None):
        ...
        if self.multi_head:
            ...
            loss=torch.zeros(1).cuda()
            for i in range(self.head_num):
                loss += p[:,i]*self.loss_criterion(output[:,2*i:2*i+2,:,:,:], target,)
...
class RS3DTrainer:
    ...
    def train(self, train_loader):
        ...
        for i, t in enumerate(train_loader):
            ...
            target = target.cuda()
            Ls1, Ls2, Lr1, Lr2, Li,S0,I1,S1,I2,S2,S3 = self._forward_pass(input, target)
    ...
    def validate(self, val_loader):
        ...
        with torch.no_grad():
            for i, t in enumerate(val_loader):
                ...
                if i % 100 == 0:
                    self._log_images(input, target,S0, I1,S1,I2,S2,S3, 'val_')
    ...
    def _forward_pass(self, input, target):
        # forward pass
        Ls1, Ls2, Lr1, Lr2, Li,S0,I1,S1,I2,S2,S3 = self.model(input,target,self.baseline)
        # compute the loss
        #loss=Ls1+Ls2+Lr1+Lr2+Li
